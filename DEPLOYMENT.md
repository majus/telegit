...
1. **Install Dokploy** (if self-hosting):
...
```bash
# Via Dokploy CLI or web interface
dokploy project create telegit
```

3. **Configure Secrets**:

Navigate to Dokploy dashboard and add the following secrets:
...
Deployment is automated via GitHub Actions. Configure these secrets in your repository:

- `DOKPLOY_API_KEY_STAGING`
- `DOKPLOY_API_KEY_PRODUCTION`
- `DOKPLOY_URL`
...
# Select environment (staging/production) and version
```
...
- Push to registry (except PRs)
- Security scan (Trivy)
- Test image with smoke tests
...
- Wait for deployment
- Run smoke tests
- Run load tests (production only)
- Rollback on failure
...
docker images telegit

# Or check GitHub Container Registry
docker pull ghcr.io/majus/telegit:previous-version
```

#### 2. Stop Current Version

```bash
docker stop telegit-app
docker rm telegit-app
```
...
docker run -d \
  --name telegit-app \
  --env-file .env \
  ghcr.io/majus/telegit:previous-version
```
...
Deployment workflow automatically rolls back on failure.

#### Manual Rollback via API

```bash
curl -X POST "https://your-dokploy.com/api/v1/rollback" \
...
```bash
dokploy rollback --project telegit --env production
```

#### Manual Rollback via UI

1. Navigate to Dokploy dashboard
2. Select TeleGit project
...
# Restore database
psql -h localhost -U postgres -d telegit < db/backups/telegit_backup_2024-01-15.sql

# Start application
docker start telegit-app
```

#### 2. Run Reverse Migrations

```bash
# If migrations support rollback
...
- [ ] Create backup of current state (database, config)
