...
When working on security-sensitive code, follow these principles:

**Encryption & Token Management:**
- Always use `src/utils/encryption.js` for encrypting/decrypting sensitive data
- Never log or expose GitHub PATs, encryption keys, or other secrets
- Use AES-256-GCM for all encryption operations (already implemented)
- Generate random IVs for each encryption operation
- Validate all encrypted data integrity using authentication tags

**Input Sanitization:**
- Always use `src/utils/sanitize.js` functions before processing user input
- Sanitize ALL Telegram message content before creating GitHub issues
- Never trust user input - assume it's malicious until sanitized
- Preserve markdown formatting but remove dangerous HTML/JavaScript
- Validate message lengths to prevent abuse

**Access Control:**
- Use `src/services/telegram/whitelist.js` for group/user access control
- Check whitelists before processing any messages
- Default to secure (deny) if whitelist configuration is ambiguous
- Log access control violations for monitoring

**Environment Variables:**
- Never hardcode secrets in code
- Always validate required environment variables on startup
- Use descriptive error messages when env vars are missing
- Document all required environment variables in `.env.example`

**Testing Security Features:**
- Write tests for XSS attack vectors
